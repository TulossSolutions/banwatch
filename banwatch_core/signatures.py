...
SERVICE_SIGNATURES = {
    "ssh": {
        "patterns": [
            ...
            {"pattern": r"Connection closed by (?:authenticating user|invalid user) \S+ (?P<ip>\d+\.\d+\.\d+\.\d+) port", "severity": "high"},
            {"pattern": r"pam_unix\(sshd:auth\): authentication failure.*rhost=(?P<ip>\d+\.\d+\.\d+\.\d+)", "severity": "high"},
            {"pattern": r"banner exchange: Connection from (?P<ip>\d+\.\d+\.\d+\.\d+) port \d+: invalid format", "severity": "medium"},
            {"pattern": r"Connection reset by (?P<ip>\d+\.\d+\.\d+\.\d+) port \d+", "severity": "medium"},
    ...
    "web": {
        "patterns": [
            ...
            {"pattern": r"(?P<ip>\d+\.\d+\.\d+\.\d+).*\".*(\.\./|\.\.\\|%2e%2e|\.%2e|%%32%65|%c0%ae|/etc/passwd).*\"", "severity": "high"},
            {"pattern": r"(?P<ip>\d+\.\d+\.\d+\.\d+).*eval-stdin\.php", "severity": "high"},
            {"pattern": r"(?P<ip>\d+\.\d+\.\d+\.\d+).*\"(GET|POST)[^\"]*(allow_url_include|auto_prepend_file|php://input)", "severity": "high"},
            {"pattern": r"(?P<ip>\d+\.\d+\.\d+\.\d+).*\"CONNECT [^\"]+:\d+ HTTP", "severity": "high"},
            {"pattern": r"(?P<ip>\d+\.\d+\.\d+\.\d+).*\"(GET|POST)[^\"]*(/\.env(\.\w+)?\b|\.git/(config|HEAD)|aws_config|aws-secret|azure-credential|gcp-key|tfstate|selfsigned|gitlab-secret|secrets\.ya?ml|claude_desktop_config|elasticsearch\.ya?ml|mailcow\.conf|docker-compose[^\"]*\.ya?ml|wp-config)", "severity": "high"},
            {"pattern": r"(?P<ip>\d+\.\d+\.\d+\.\d+).*%2e(?:env|git|config|ini|yml|yaml|sql|json|key|log|tfstate|lock|info|bak|tmp|php|ts|xml)", "severity": "high"},
            {"pattern": r"(?P<ip>\d+\.\d+\.\d+\.\d+).*\"(GET|POST)[^\"]*/?\.(?:openclaw|cursor|aider|continue|codex|claude|hermes|mcp)\b", "severity": "high"},
            {"pattern": r"(?P<ip>\d+\.\d+\.\d+\.\d+).*\"(GET|POST)[^\"]*/?\.(?:bashrc|bash_profile|zshrc|profile|htaccess|htpasswd|envrc|amplifyrc)\b", "severity": "high"},
            {"pattern": r"(?P<ip>\d+\.\d+\.\d+\.\d+).*(invokefunction|call_user_func_array|pearcmd)", "severity": "high"},
            {"pattern": r"(?P<ip>\d+\.\d+\.\d+\.\d+).*phpinfo", "severity": "medium"},
            {"pattern": r"(?P<ip>\d+\.\d+\.\d+\.\d+).*(wlwmanifest\.xml|xmlrpc\.php)", "severity": "medium"},
            {"pattern": r"(?P<ip>\d+\.\d+\.\d+\.\d+).*XDEBUG_SESSION_START", "severity": "medium"},
            {"pattern": r"open\(\) \"(?!.*404\.html)[^\"]*\" failed \(2: No such file or directory\).*client: (?P<ip>\d+\.\d+\.\d+\.\d+)", "severity": "low"},
            {"pattern": r"access forbidden by rule.*client: (?P<ip>\d+\.\d+\.\d+\.\d+)", "severity": "low"},
            {"pattern": r"(?P<ip>\d+\.\d+\.\d+\.\d+).*mstshash=", "severity": "medium"},
